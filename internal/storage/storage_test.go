...
// Test user ID for all in-memory storage tests
var testMemoryUserID = uuid.MustParse("22222222-2222-2222-2222-222222222222")
...
func TestCreateList(t *testing.T) {
	...
	t.Run("successfully creates a list", func(t *testing.T) {
		...
		list, err := store.CreateList(testMemoryUserID, req)
	...
	t.Run("fails when list name already exists", func(t *testing.T) {
		...
		_, err := store.CreateList(testMemoryUserID, req)
		require.NoError(t, err)
...
func TestGetAllLists(t *testing.T) {
	...
	t.Run("returns paginated lists", func(t *testing.T) {
		lists, pagination, err := store.GetAllLists(testMemoryUserID, 1, 10)
		require.NoError(t, err)
	...
	t.Run("returns correct page", func(t *testing.T) {
		lists, pagination, err := store.GetAllLists(testMemoryUserID, 2, 10)
		require.NoError(t, err)
		assert.Len(t, lists, 10)
	...
	t.Run("returns last page with remaining items", func(t *testing.T) {
		lists, pagination, err := store.GetAllLists(testMemoryUserID, 3, 10)
		require.NoError(t, err)
...
func TestGetListByID(t *testing.T) {
	...
	created, err := store.CreateList(testMemoryUserID, req)
	require.NoError(t, err)

	t.Run("successfully retrieves list", func(t *testing.T) {
		list, err := store.GetListByID(testMemoryUserID, created.ID)
		require.NoError(t, err)
	...
	t.Run("fails when list not found", func(t *testing.T) {
		_, err := store.GetListByID(testMemoryUserID, uuid.New())
...
func TestUpdateList(t *testing.T) {
	...
	t.Run("successfully updates name", func(t *testing.T) {
		...
		updated, err := store.UpdateList(testMemoryUserID, created.ID, updateReq)
		require.NoError(t, err)
	...
	t.Run("fails when list not found", func(t *testing.T) {
		...
		_, err := store.UpdateList(testMemoryUserID, uuid.New(), updateReq)
		assert.ErrorIs(t, err, ErrListNotFound)
	})
	...
	t.Run("fails when new name conflicts", func(t *testing.T) {
		...
		_, err := store.CreateList(testMemoryUserID, req2)
		require.NoError(t, err)
		...
		_, err = store.UpdateList(testMemoryUserID, created.ID, updateReq)
		assert.ErrorIs(t, err, ErrListNameExists)
	})
}
...
func TestDeleteList(t *testing.T) {
	...
	_, err = store.CreateTodo(testMemoryUserID, created.ID, todoReq)
	require.NoError(t, err)

	t.Run("successfully deletes list and todos", func(t *testing.T) {
		err := store.DeleteList(testMemoryUserID, created.ID)
		require.NoError(t, err)

		// Verify list is deleted
		_, err = store.GetListByID(testMemoryUserID, created.ID)
		assert.ErrorIs(t, err, ErrListNotFound)
	...
	t.Run("fails when list not found", func(t *testing.T) {
		err := store.DeleteList(testMemoryUserID, uuid.New())
		assert.ErrorIs(t, err, ErrListNotFound)
	})
}
...
func TestCreateTodo(t *testing.T) {
	...
	t.Run("successfully creates todo", func(t *testing.T) {
		...
		todo, err := store.CreateTodo(testMemoryUserID, list.ID, todoReq)
		require.NoError(t, err)
	...
	t.Run("fails when list not found", func(t *testing.T) {
		...
		_, err := store.CreateTodo(testMemoryUserID, uuid.New(), todoReq)
		assert.ErrorIs(t, err, ErrListNotFound)
	})
}
...
func TestGetTodosByList(t *testing.T) {
	...
	for _, todoReq := range todos {
		_, err := store.CreateTodo(testMemoryUserID, list.ID, todoReq)
		require.NoError(t, err)
	}
...
func TestUpdateTodo(t *testing.T) {
	...
	t.Run("marks todo as completed", func(t *testing.T) {
		...
		updated, err := store.UpdateTodo(testMemoryUserID, list.ID, todo.ID, updateReq)
		require.NoError(t, err)
	...
	t.Run("fails when list not found", func(t *testing.T) {
		updateReq := models.UpdateTodoRequest{}
		_, err := store.UpdateTodo(testMemoryUserID, uuid.New(), todo.ID, updateReq)
		assert.ErrorIs(t, err, ErrListNotFound)
	})

	t.Run("fails when todo not found", func(t *testing.T) {
		updateReq := models.UpdateTodoRequest{}
		_, err := store.UpdateTodo(testMemoryUserID, list.ID, uuid.New(), updateReq)
...
func TestDeleteTodo(t *testing.T) {
	...
	t.Run("successfully deletes todo", func(t *testing.T) {
		err := store.DeleteTodo(testMemoryUserID, list.ID, todo.ID)
		require.NoError(t, err)

		_, err = store.GetTodoByID(testMemoryUserID, list.ID, todo.ID)
		assert.ErrorIs(t, err, ErrTodoNotFound)
	})

	t.Run("fails when list not found", func(t *testing.T) {
		err := store.DeleteTodo(testMemoryUserID, uuid.New(), uuid.New())
		assert.ErrorIs(t, err, ErrListNotFound)
	})

	t.Run("fails when todo not found", func(t *testing.T) {
		err := store.DeleteTodo(testMemoryUserID, list.ID, uuid.New())
		assert.ErrorIs(t, err, ErrTodoNotFound)
	})
}
